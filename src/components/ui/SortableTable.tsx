...
/**
 * Column-header sorting for data tables.
 *
 * `getValue` maps a row + column key to a scalar to compare. Rows keep their
 * incoming order until a header is clicked (sortKey starts null), so whatever
 * ordering the backend chose survives the first render.
 */
export function useTableSort<T>(
  ...
  const sorted = useMemo(() => {
    ...
    return [...rows].sort((a, b) => {
      ...
      // Missing values always sort to the bottom, whichever way the column runs.
...
/** A clickable `<th>` whose arrow reflects the active sort direction. */
