...
type Metric = "volume" | "publishedRate" | "avgLeadTimeHours";

const METRIC_CONFIG: { key: Metric; label: string; suffix?: string; lowerIsBetter?: boolean }[] = [
  { key: "volume", label: "Allotment Volume" },
  { key: "publishedRate", label: "Published Rate", suffix: "%" },
  { key: "avgLeadTimeHours", label: "Lead Time", suffix: "h", lowerIsBetter: true },
];

export default function AllotterComparisonChart({ data, isLoading }: Props) {
  ...
  const { chartData, teamAvg } = useMemo(() => {
    ...
    const values = sorted.map((a) => a[metric] as number);
    const avg = values.length > 0 ? values.reduce((s, v) => s + v, 0) / values.length : 0;

    return {
      chartData: sorted.map((a) => ({
        name: a.allottedBy,
        value: a[metric] as number,
        aboveAvg: (a[metric] as number) >= avg,
      })),
  ...
  if (isLoading || !data?.length) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
        <h3 className="text-sm font-bold text-gray-900 dark:text-white">Allotter Performance vs Baseline</h3>
      ...
      <div className="h-56">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
            ...
            />
            <ReferenceLine y={teamAvg} stroke="#f59e0b" strokeDasharray="4 4" strokeWidth={2} />
