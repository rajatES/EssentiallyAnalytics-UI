...
export default function EditorDailyBreakdown({ data, isLoading }: Props) {
  ...
  const gridData = useMemo(() => {
    if (!data?.daily || !allDates.length) return { editors: [] as string[], dates: [] as string[], map: new Map<string, Map<string, DailyBreakdownEntry>>() };
    ...
    let editorList = [...map.keys()].sort();
    if (search) {
      const q = search.toLowerCase();
      editorList = editorList.filter((e) => e.toLowerCase().includes(q));
    }
    return { editors: editorList, dates: allDates, map };
  ...
  const { progressChartData, rankedEditors, editorTotals } = useMemo(() => {
    if (!data?.daily?.length) return { progressChartData: [] as Record<string, string | number>[], rankedEditors: [] as string[], editorTotals: new Map<string, number>() };
    ...
    const cd = sortedDates.map((date) => {
      ...
      for (const e of eSet) entry[e] = dayMap.get(e) || 0;
      return entry;
    });
    const ranked = [...eSet].sort((a, b) => (totals.get(b) || 0) - (totals.get(a) || 0));
    return { progressChartData: cd, rankedEditors: ranked, editorTotals: totals };
  }, [data]);

  const activeProgressEditors = progressSelected.length > 0 ? progressSelected : rankedEditors.slice(0, 10);

  const filteredProgressDropdown = useMemo(() => {
    if (!progressSearch) return rankedEditors;
    const q = progressSearch.toLowerCase();
    return rankedEditors.filter((e) => e.toLowerCase().includes(q));
  }, [rankedEditors, progressSearch]);

  const toggleProgressEditor = (e: string) => {
    setProgressSelected((prev) => prev.includes(e) ? prev.filter((x) => x !== e) : [...prev, e]);
  };
  const selectProgressTop = (n: number) => {
    setProgressSelected(rankedEditors.slice(0, n));
  ...
  const chartData = useMemo(() => {
    if (!data?.daily || !selectedEditor) return [];
    return data.daily
      .filter((r) => r.name === selectedEditor)
  ...
  const personalAvgForEditor = useMemo(() => {
    if (!selectedEditor) return null;
    return avgMap.get(selectedEditor) ?? null;
  }, [avgMap, selectedEditor]);
  ...
  const exportCSV = useCallback(() => {
    ...
    const header = "Editor,Date,Slides Edited,Slideshows Edited,Articles Edited,Total\n";
    const rows = data.daily.map((r) => `"${r.name}","${r.date}",${r.slides},${r.slideshows},${r.articles},${r.total}`).join("\n");
    const blob = new Blob([header + rows], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "editor_daily_breakdown.csv"; a.click();
  ...
  if (isLoading || !data) {
    return (
      <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
        <h3 className="text-sm font-bold text-gray-900 dark:text-white">Editor Daily Breakdown</h3>
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
        ...
        <div className="flex items-center gap-2">
            ...
            {TABS.map((t, i) => (
              <button
                ...
                className={`inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-medium transition-colors ${
                  tab === t.key ? "bg-violet-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-300"
          ...
          {tab !== "progress" && (
            <div className="relative">
              ...
              <input type="text" placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} className="rounded-md border border-gray-300 bg-white py-1.5 pl-8 pr-3 text-xs outline-none focus:ring-1 focus:ring-violet-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200" />
            </div>
          )}
          <button onClick={exportCSV} className="inline-flex items-center gap-1 rounded-md border border-gray-300 bg-white px-2.5 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300">
      ...
      {/* Team Average Banner */}
      <div className="mb-4 flex flex-wrap gap-3 rounded-lg bg-gradient-to-r from-violet-50 to-purple-50 p-3 dark:from-violet-900/20 dark:to-purple-900/20">
        <div className="flex items-center gap-1.5">
          <Users size={14} className="text-violet-500" />
          <span className="text-[11px] font-semibold text-violet-700 dark:text-violet-300">Team Avg/Day:</span>
        </div>
        {[
          { v: teamAvg.avgSlides, l: "slides edited" },
          { v: teamAvg.avgSlideshows, l: "SS edited" },
          { v: teamAvg.avgArticles, l: "articles edited" },
        ].map((m) => (
          <span key={m.l} className="rounded-full bg-white/70 px-2 py-0.5 text-[11px] font-medium text-gray-700 dark:bg-gray-800/50 dark:text-gray-300">{m.v} {m.l}</span>
        ))}
        <span className="rounded-full bg-violet-100 px-2 py-0.5 text-[11px] font-bold text-violet-700 dark:bg-violet-800/50 dark:text-violet-300">{teamAvg.avgTotal} total</span>
      </div>
      ...
      {tab === "snapshot" && (
        <div>
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-[10px] text-gray-400 dark:text-gray-500">{snapshotRows.length} editor{snapshotRows.length !== 1 ? "s" : ""} active</span>
          </div>
          {snapshotRows.length === 0 ? (
            <div className="flex h-32 items-center justify-center text-sm text-gray-400">No editor data for this date.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50">
                    {([["name", "Editor"], ["slides", "Slides Edited"], ["slideshows", "SS Edited"], ["articles", "Articles Edited"], ["total", "Total"]] as [SnapshotSortKey, string][]).map(([k, label]) => (
                  ...
                  {snapshotRows.map((r) => {
                    ...
                    return (
                      <tr key={r.name} className="border-b border-gray-50 hover:bg-gray-50/50 dark:border-gray-800 dark:hover:bg-gray-800/30">
                        ...
                        <td className="px-3 py-2 text-violet-600 dark:text-violet-400">{r.slideshows}</td>
                        <td className="px-3 py-2 text-emerald-600 dark:text-emerald-400">{r.articles}</td>
                        <td className="px-3 py-2 font-medium text-violet-600 dark:text-violet-400">{r.total}</td>
                ...
                <tfoot>
                  <tr className="border-t-2 border-gray-300 bg-gray-50 font-semibold dark:border-gray-600 dark:bg-gray-800/50">
                    ...
                    <td className="px-3 py-2 text-violet-600 dark:text-violet-400">{snapshotRows.reduce((s, r) => s + r.slideshows, 0)}</td>
                    <td className="px-3 py-2 text-emerald-600 dark:text-emerald-400">{snapshotRows.reduce((s, r) => s + r.articles, 0)}</td>
                    <td className="px-3 py-2 text-violet-600 dark:text-violet-400">{snapshotRows.reduce((s, r) => s + r.total, 0)}</td>
                    <td colSpan={2} />
                  </tr>
                </tfoot>
              </table>
      ...
      {tab === "grid" && (
        <div>
          <p className="mb-2 text-[10px] text-gray-400 dark:text-gray-500">Each cell shows total edited. Hover for breakdown. Color: <span className="text-emerald-600">green</span> = above personal avg, <span className="text-red-500">red</span> = below.</p>
            ...
            <table className="w-max min-w-full text-[11px]">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800/50">
                  <th className="sticky left-0 z-10 min-w-[120px] border-r border-gray-200 bg-gray-50 px-3 py-2 text-left text-xs font-medium text-gray-500 dark:border-gray-700 dark:bg-gray-800/50 dark:text-gray-400">Editor</th>
                  <th className="sticky left-[120px] z-10 min-w-[48px] border-r border-gray-200 bg-gray-50 px-2 py-2 text-center text-[10px] font-semibold text-violet-600 dark:border-gray-700 dark:bg-gray-800/50 dark:text-violet-400">Avg</th>
              ...
              <tbody>
                {gridData.editors.map((editor) => {
                  ...
                  return (
                    <tr key={editor} className="border-t border-gray-50 hover:bg-gray-50/30 dark:border-gray-800/50 dark:hover:bg-gray-800/20">
                      <td className="sticky left-0 z-10 border-r border-gray-200 bg-white px-3 py-1.5 font-semibold text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-white">{editor}</td>
                      <td className="sticky left-[120px] z-10 border-r border-gray-200 bg-white px-2 py-1.5 text-center font-bold text-violet-600 dark:border-gray-700 dark:bg-gray-900 dark:text-violet-400">{avg > 0 ? avg.toFixed(1) : "—"}</td>
                      {gridData.dates.map((d) => {
                        const entry = editorDays.get(d);
                            ...
                            <div className={`group relative inline-flex min-w-[28px] items-center justify-center rounded px-1 py-0.5 font-bold ${bg} ${text}`}>
                              ...
                              <div className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 hidden -translate-x-1/2 rounded-lg border border-gray-200 bg-white px-3 py-2 shadow-xl group-hover:block dark:border-gray-700 dark:bg-gray-900">
                                <div className="whitespace-nowrap text-[11px]">
                                  <div className="mb-1 font-bold text-gray-900 dark:text-white">{editor} — {d}</div>
                                  ...
                                  <div className="text-violet-500">Slideshows: <span className="font-semibold">{entry.slideshows}</span></div>
                                  <div className="text-emerald-500">Articles: <span className="font-semibold">{entry.articles}</span></div>
                                  <div className="mt-1 border-t border-gray-100 pt-1 font-bold text-violet-600 dark:border-gray-800 dark:text-violet-400">Total: {entry.total}</div>
        ...
        <div>
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {[5, 10, 15].map((n) => (
                <button key={n} onClick={() => selectProgressTop(n)} className={`rounded px-2 py-0.5 text-[10px] font-medium transition-colors ${progressSelected.length === n && progressSelected.every((e, i) => e === rankedEditors[i]) ? "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400" : "bg-gray-100 text-gray-500 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700"}`}>
                  Top {n}
                </button>
              ))}
            </div>
            <div className="relative">
              <button onClick={() => setProgressDropdownOpen(!progressDropdownOpen)} className="flex items-center gap-1.5 rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
                <Users size={12} />
                {progressSelected.length > 0 ? `${progressSelected.length} selected` : `Top ${activeProgressEditors.length}`}
                ...
                <div className="absolute right-0 top-full z-30 mt-1 w-72 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-900">
                  <div className="border-b border-gray-100 p-2 dark:border-gray-800">
                    <div className="relative">
                      ...
                      <input type="text" placeholder="Search editors..." value={progressSearch} onChange={(e) => setProgressSearch(e.target.value)} className="w-full rounded-md border border-gray-200 bg-gray-50 py-1.5 pl-8 pr-3 text-xs outline-none focus:ring-1 focus:ring-violet-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200" autoFocus />
                    </div>
                  </div>
                  <div className="flex gap-1 border-b border-gray-100 px-2 py-1.5 dark:border-gray-800">
                    <button onClick={() => { setProgressSelected([]); setProgressDropdownOpen(false); }} className="rounded px-2 py-0.5 text-[10px] font-medium text-violet-600 hover:bg-violet-50 dark:text-violet-400 dark:hover:bg-gray-800">All (Top 10)</button>
                    ...
                    <button onClick={() => { setProgressSelected(rankedEditors); setProgressDropdownOpen(false); }} className="rounded px-2 py-0.5 text-[10px] font-medium text-gray-500 hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-gray-800">Select All</button>
                  ...
                  <div className="max-h-60 overflow-y-auto p-1.5">
                    {filteredProgressDropdown.map((e) => {
                      const isSelected = progressSelected.includes(e);
                      const total = editorTotals.get(e) || 0;
                      const colorIdx = activeProgressEditors.indexOf(e);
                      return (
                        <button key={e} onClick={() => toggleProgressEditor(e)} className={`flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs transition-colors ${isSelected ? "bg-violet-50/50 font-semibold text-gray-900 dark:bg-violet-900/20 dark:text-white" : "text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-gray-800"}`}>
          ...
          {progressSelected.length > 0 && (
            <div className="mb-3 flex flex-wrap gap-1">
              {progressSelected.map((e, i) => (
                <span key={e} onClick={() => toggleProgressEditor(e)} className="inline-flex cursor-pointer items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold transition-colors hover:opacity-80" style={{ backgroundColor: `${LINE_COLORS[i % LINE_COLORS.length]}15`, color: LINE_COLORS[i % LINE_COLORS.length], border: `1px solid ${LINE_COLORS[i % LINE_COLORS.length]}30` }}>
                  <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: LINE_COLORS[i % LINE_COLORS.length] }} />{e}<X size={9} />
                </span>
              ))}
            </div>
          )}
          ...
          <div className="h-[380px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={progressChartData} margin={{ top: 10, right: 10, left: -10, bottom: 5 }} onMouseLeave={() => setHoveredKey(null)}>
                ...
                {activeProgressEditors.map((e, i) => (
                  <Line key={e} type="monotone" dataKey={e} stroke={LINE_COLORS[i % LINE_COLORS.length]} strokeWidth={hoveredKey === e ? 3 : hoveredKey === null ? 2 : 1} strokeOpacity={hoveredKey === null || hoveredKey === e ? 1 : 0.15} dot={progressChartData.length <= 15 ? { r: 2.5 } : false} activeDot={hoveredKey === e ? { r: 6, strokeWidth: 2, stroke: "#fff" } : false} isAnimationActive={false} />
                ))}
                {activeProgressEditors.map((e) => (
                  <Line key={`${e}-hover`} type="monotone" dataKey={e} stroke="transparent" strokeWidth={30} dot={false} activeDot={false} isAnimationActive={false} onMouseEnter={() => setHoveredKey(e)} onMouseLeave={() => setHoveredKey(null)} />
                ))}
              </LineChart>
            </ResponsiveContainer>
          </div>
      ...
      {tab === "averages" && (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50">
                {([["name", "Editor"], ["avgSlides", "Avg Slides/Day"], ["avgSlideshows", "Avg SS/Day"], ["avgArticles", "Avg Articles/Day"], ["avgTotal", "Avg Total/Day"], ["activeDays", "Active Days"]] as [AvgSortKey, string][]).map(([k, label]) => (
              ...
              {filteredAvg.map((p) => {
                ...
                return (
                  <tr key={p.name} className="border-b border-gray-50 hover:bg-gray-50/50 dark:border-gray-800 dark:hover:bg-gray-800/30">
                    ...
                    <td className="px-3 py-2 text-violet-600 dark:text-violet-400">{p.avgSlideshows}</td>
                    <td className="px-3 py-2 text-emerald-600 dark:text-emerald-400">{p.avgArticles}</td>
                    <td className="px-3 py-2 font-medium text-violet-600 dark:text-violet-400">{p.avgTotal}</td>
            ...
            <tfoot>
              <tr className="border-t-2 border-gray-300 bg-gray-50 font-semibold dark:border-gray-600 dark:bg-gray-800/50">
                ...
                <td className="px-3 py-2 text-violet-600 dark:text-violet-400">{teamAvg.avgSlideshows}</td>
                <td className="px-3 py-2 text-emerald-600 dark:text-emerald-400">{teamAvg.avgArticles}</td>
                <td className="px-3 py-2 text-violet-600 dark:text-violet-400">{teamAvg.avgTotal}</td>
      ...
      {tab === "charts" && (
        <div className="space-y-4">
          {selectedEditor && chartData.length > 0 ? (
            <div>
              <h4 className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300"><TrendingUp size={14} /> {selectedEditor} — Daily Editing Output</h4>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                    ...
                    {personalAvgForEditor && <ReferenceLine y={personalAvgForEditor.avgTotal} stroke="#10b981" strokeDasharray="4 4" strokeWidth={2} label={{ value: `Avg: ${personalAvgForEditor.avgTotal}`, position: "insideTopRight", fill: "#10b981", fontSize: 10 }} />}
                    ...
                    <Line type="monotone" dataKey="articles" stroke="#10b981" strokeWidth={2} dot={{ r: 3 }} name="Articles Edited" />
                    <Line type="monotone" dataKey="slideshows" stroke="#8b5cf6" strokeWidth={2} dot={{ r: 3 }} name="SS Edited" />
                    <Line type="monotone" dataKey="slides" stroke="#a78bfa" strokeWidth={1} dot={false} name="Slides Edited" strokeDasharray="3 3" />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
          ) : (
            !selectedEditor && <p className="mb-2 text-xs text-gray-400">Select an editor above to see their daily trend chart.</p>
          )}
          <div>
            <h4 className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-gray-700 dark:text-gray-300"><Users size={14} /> Editor Comparison — Avg Total Edited/Day (Top 25)</h4>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={comparisonData} layout="vertical" margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="avgSlideshows" name="Slideshows" fill="#8b5cf6" stackId="a" barSize={14} radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
  ...
  if (hoveredKey) {
    ...
    return (
      <div className="rounded-lg border border-gray-200 bg-white px-3 py-2.5 shadow-lg dark:border-gray-700 dark:bg-gray-800 min-w-[140px]">
        ...
        <p className="text-lg font-bold text-gray-900 dark:text-white mt-0.5">{entry.value} edited</p>
      </div>
    );
  }
